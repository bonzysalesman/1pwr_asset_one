<?php
/*
Template Name: Custom Asset List with Allocation Actions
*/
...
$query = "SELECT a.*, c.name as category_name, 
          COALESCE(al.employee_name, 'Unallocated') as allocated_to,
          COALESCE(al.department_name, '') as department_name
          FROM assets a 
          LEFT JOIN categories c ON a.category_id = c.category_id
          LEFT JOIN (
              SELECT a.asset_id, e.name as employee_name, d.short_name as department_name
              FROM allocations a
...
<div class="py-4">
    ...
    <div class="d-flex justify-content-between w-100 flex-wrap">
        ...
        <div>
            <a href="<?php echo get_permalink(get_page_by_path('add-asset')); ?>" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
        ...
        <form method="get" class="row g-3">
            ...
            <div class="col-auto">
                ...
                <?php if (!empty($search_term) || !empty($category_filter)) : ?>
                    <a href="<?php echo remove_query_arg(['search', 'category']); ?>" class="btn btn-light">Clear</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <tbody>
            <?php if ($assets) : ?>
                <?php foreach ($assets as $asset) : ?>
                    <tr>
                        <td><span class="fw-normal"><?php echo esc_html($asset->name); ?></span></td>
                        ...
                        <td><span class="fw-normal"><?php echo esc_html($asset->allocated_to); ?></span></td>
                        <td><span class="fw-normal"><?php echo esc_html($asset->department_name); ?></span></td>
                        <td>
                            <div class="btn-group">
                                <a href="<?php echo get_permalink(get_page_by_path('asset')) . '?asset_id=' . $asset->asset_id; ?>" 
                                ...
                                <a href="<?php echo get_permalink(get_page_by_path('edit-asset')) . '?asset_id=' . $asset->asset_id; ?>" 
                                ...
                                <a href="<?php echo get_permalink(get_page_by_path('delete-asset')) . '?asset_id=' . $asset->asset_id; ?>" 
                                   class="btn btn-link text-dark p-0" title="Delete Asset" onclick="return confirm('Are you sure you want to delete this asset?')">
                                    <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M4 6h12M5 6l1 12a2 2 0 001.85 2h5.3a2 2 0 001.85-2L15 6H5z"></path>
                                    </svg>
                                </a>
                            </div>
                        </td>
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination mb-0">
                    <?php if ($current_page > 1) : ?>
                        <li class="page-item">
                            <a class="page-link" href="<?php echo add_query_arg('paged', $current_page - 1); ?>">Previous</a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php if ($current_page < $total_pages) : ?>
                        <li class="page-item">
                            <a class="page-link" href="<?php echo add_query_arg('paged', $current_page + 1); ?>">Next</a>
                        </li>
                    <?php endif; ?>
                </ul>
            </nav>
