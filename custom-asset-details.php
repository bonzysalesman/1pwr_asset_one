<?php
/*
Template Name: Custom Asset Details
*/
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_asset'])) {
    ...
    if (isset($_POST['asset_id'])) {
        ...
        $result = $wpdb->update(
            'assets',  // Added table prefix
            $asset_values,
            ['asset_id' => intval($_POST['asset_id'])],
            ['%s', '%s', '%s', '%s', '%s', '%d'],  // Added format parameters
            ['%d']
        );
        ...
        if ($result !== false) {
            ...
            $error = 'Error updating asset: ' . $wpdb->last_error;
        }
    } else {
        ...
        if ($result) {
            ...
            $error = 'Error adding asset: ' . $wpdb->last_error;
        }
    }
}
...
<div class="card card-body border-0 shadow mb-4">
    <div class="d-flex justify-content-between">
        ...
        <?php if (isset($_GET['asset_id'])) : ?>
            <a href="<?php echo get_permalink(get_page_by_path('asset-history')) . '?asset_id=' . intval($_GET['asset_id']); ?>" 
